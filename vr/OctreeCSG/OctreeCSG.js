...
OctreeCSG.unionArray = function (objArr, buildTargetOctree = true) {
    ...
    for (let i = 0; i < objArr.length; i++) {
        let tempOctree
        if (objArr[i].isMesh) {
            tempOctree = OctreeCSG.fromMesh(objArr[i], i);
        }
        else {
            tempOctree = objArr[i];
            tempOctree.setPolygonIndex(i);
        }
        tempOctree.octreeIndex = i;
        octreesArray.push(tempOctree);
    }
    let resultOctreesArray = [];
    let octree = new OctreeCSG();

    // Handle the first Octree
    let octreeA = octreesArray[0].clone();
    // let octreeA_clone = octreeA.clone();
    for (let i = 1; i < octreesArray.length; i++) {
        let octreeB = octreesArray[i];
        if (octreeA.box.intersectsBox(octreeB.box)) {
            octreeA.resetPolygons();
            octreeA.markPolygonsAsOriginal();
            // octreeB.resetPolygons(false);

            octreeA.markIntesectingPolygons(octreeB);
            // octreeB.markIntesectingPolygons(octreeA);

            handleIntersectingOctrees(octreeA, octreeB, false);
            octreeA.deleteReplacedPolygons();
            // octreeB.deleteReplacedPolygons();

            octreeA.deletePolygonsByStateRules(unionRules.a);

            // octreeB.deletePolygonsByStateRules(unionRules.b);
    ...
    resultOctreesArray.push(octreeA);

    // Handle the rest of the Octrees
    let octreeStack = octreesArray.slice(1, octreesArray.length);
    let octreeB = octreeStack.pop();
    while (octreeB) {
        octreeB = octreeB.clone();
        for (let i = 0; i < octreesArray.length; i++) {
            let octreeA = octreesArray[i];
            if (octreeA.box.intersectsBox(octreeB.box)) {
                octreeB.resetPolygons();
                octreeB.markPolygonsAsOriginal();
                
                // octreeA.markIntesectingPolygons(octreeB);
                octreeB.markIntesectingPolygons(octreeA);

                handleIntersectingOctrees(octreeB, octreeA, false);
                // octreeA.deleteReplacedPolygons();
                octreeB.deleteReplacedPolygons();
        ...
        resultOctreesArray.push(octreeB);
        octreeB = octreeStack.pop();
    }



    for (let i = 0; i < resultOctreesArray.length; i++) {
        resultOctreesArray[i].getPolygonCloneCallback(octree.addPolygon.bind(octree));
        resultOctreesArray[i].delete(false);
    }
...
function getAllAvailableWorkers() {
    return workerPool.filter(worker => !worker.running);
}
...
/*
// TODO:
Jobs pool with random UUID for job ID and tasks pool inside it (const jobs = [{id: 1, callbackFunc: func, tasks: [...], workerCount: 0}, {id: 2, callbackFunc: func2, tasks[...], workerCount: 0}])
1. Call the createWorkers function
2. Logic will push all required jobs / tasks to the pool
3. Logic will call the workers and mark them for the current job and increase the workerCount for the job
4. Workers will start working on tasks
5. Once a worker finished working on a task, it will check for another task in the job pool based on the job ID it was marked with
6. If all tasks are finished, reduce the workerCount on the job and then check if another job is available to work on and remark itself with the new job ID and start working on the tasks
7. Once the last worker finishes working on the job (workerCount == 0), call the callback function for the job
*/
class GenerateWebWorker {
    ...
    checkWindingNumber(data, currentPolygon) {
        ...
        if (this.worker === null) {
            throw new Error(`[GenerateWebWorker] checkWindingNumber: Worker has been disposed`);
        }
        this.running = true;
        const { worker } = this;
